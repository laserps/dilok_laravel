@extends('welcome')
@section('body')
<div class="wrapper">

        <!-- CART SIDEBAR -->
        @include('cart-sidebar')
        @include('nav-sidebar')
        <!-- END CART SIDEBAR -->

  <div class="custom-container  px-0">
      <div class="row mt-4 row_fix">
        <div class="col-lg-12">
          <div class="add_our_story-fix-frame">
            <img class="add_our_story-img-top" src="https://brandinside.asia/wp-content/uploads/2017/03/adidas-1867084_960_720-e1498271284883.jpg">
          </div>
        </div>
      </div>
    </div>

    <div class="custom-container">
      <div class="row add_our_story-padding mt-lg-4 mt-1">
        <div class="col-lg-6  order-lg-0 order-2">
          <div class="add_our_story-fix-frame-boy ml-auto">
            <img class="add_our_story-img-top-boy " src="https://brandinside.asia/wp-content/uploads/2017/03/adidas-1867084_960_720-e1498271284883.jpg">
          </div>
        </div>
        <div class="col-lg-6 text-center add_our_story-padtext order-lg-0 order-1" style=" text-transform: uppercase;">
          <div class="our-story-vertical-center">
            <h4>ABOUT DILOK</h4>
            <p>Welcome to Didok It is a long established fact that a reader will be distracted by the readable is simply dummy text of the printing and typesetting industry. Lorem Ipsum</p>
          </div>
          
        </div>
      </div>
    </div>

     <div class="custom-container">
      <div class="row add_our_story-padding">
        <div class="col-lg-6 text-center add_our_story-padtext order-lg-0 order-3" style=" text-transform: uppercase;">
          <div class="our-story-vertical-center">
            <h4>ABOUT DILOK</h4>
            <p>There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the </p>
          </div>
        </div>
        <div class="col-lg-6 order-lg-0 order-4">
          <div class="add_our_story-fix-frame-boy ">
            <img class="add_our_story-img-top-boy text-center" src="https://brandinside.asia/wp-content/uploads/2017/03/adidas-1867084_960_720-e1498271284883.jpg">
          </div>
        </div>
      </div>
    </div>

    <div class="custom-container px-0">
      <div class="row mt-4 row_fix">
        <div class="col-lg-12 add_our-story_pad">
          <div class="add_our_story-fix-frame add-mat">
            <img class="add_our_story-img-top" src="http://moresneakers.com/wp-content/uploads/2017/08/20507772_10154881760207686_3961999767642757148_o.jpg">
          </div>
        </div>
      </div>
    </div>
</div>
@endsection

@section('js_bottom')

@endsection

