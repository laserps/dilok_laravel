                ...
                <div class="card-body p-0">
                  <div class="row px-0 mx-0">
                    ...
                    @if(!empty($value_product->price))
                        ...
                        @endif
                        @if($value_product->price != $special_price)
                          @php $percen_sum = (100-(($special_price*100)/$value_product->price)); @endphp
                        @else
                    ...
                    @if($date >= $special_from_date && $date <= $special_to_date)
                      @if($percen_sum != null || $percen_sum != '')
                        @foreach($value_custom as $key => $value)
                        <div id="ribbon2" class="red-ribbon" style="@if(empty($value->attributeCode == 'special_price') && !empty($special_price)) {{ 'display: none;' }} @else {{ '' }} @endif">{{number_format($percen_sum,0)}}%
                        </div>
                        @endforeach
                      @endif
                    @endif
                      ...
                      <div class="col-8 px-xl-2 px-0 mb-2 latest-product-price">
                        ...
                        @if(!empty($value_product->price))
                          <span class="@if($date >= $special_from_date && $date <= $special_to_date){{$before}}@endif">
                              {{ number_format($value_product->price,2) }}
                          </span>
                        @endif
                          <span style="@if($date >= $special_from_date && $date <= $special_to_date){{$pan}}@endif" class="@if($date >= $special_from_date && $date <= $special_to_date){{$after}}@endif">
                            @if($special_price != 0)
                              @if($date >= $special_from_date && $date <= $special_to_date)
